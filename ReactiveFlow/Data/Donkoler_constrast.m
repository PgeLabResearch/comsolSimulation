...
fileNumber = [5, 1, 2];
qValue = [0.5, 0.1, 0.02];
 tpv = [94.94, 474.71, 2373.5]; injectConcentration = [5e-6, 1e-6, 2e-7];
%% basic parameters of Breakthrough
fonts=12;
subplot(ndiagr,ndiagc,1,'position',locationsub(2,1,:));
for i = 1 : length(fileNumber)
    names=['E:\0 Comsol - t\Data\Donkoler_contrast\TCE-BreakthroughCurve\',num2str(fileNumber(i)), '.txt'];
    tc = load(names);
    if i == 5
        names=['E:\0 Comsol - t\Data\Donkoler_contrast\TCE-BreakthroughCurve\',num2str(fileNumber(i)), '-p.txt'];
        tc = load(names);
        plot(tc(:, 1)./tpv(i), tc(:, 2)./injectConcentration(i),'linewidth',1.5,...
        'DisplayName',['Q = ',num2str(qValue(i)), 'mL/min']);
    else
        plot(tc(:, 2)./tpv(i), tc(:, 3)./injectConcentration(i),'linewidth',1.5,...
        'DisplayName',[' Q = ',num2str(qValue(i)), 'mL/min']);
    end
...
ylim([0 1.5]);set(gca,'YTick',[0:0.5:2.0]);
...
fonts=12; initialMass = 2.3597e-4;
subplot(ndiagr,ndiagc,2,'position',locationsub(2,2,:));
for i = 1 : length(fileNumber)
    names=['E:\0 Comsol - t\Data\Donkoler_contrast\TCE-Retention\',num2str(fileNumber(i)), '.txt'];
    tc = load(names);
    if i == 5
        names=['E:\0 Comsol - t\Data\Donkoler_contrast\TCE-Retention\',num2str(fileNumber(i)), '-p.txt'];
        tc = load(names);
        plot(tc(:, 1)./tpv(i), tc(:, 2)./initialMass,'linewidth',1.5,...
        'DisplayName',['No. ',num2str(i)]);
    else
        plot(tc(:, 2)./tpv(i), tc(:, 3)./initialMass,'linewidth',1.5,...
        'DisplayName',['No. ',num2str(i)]);
    end
    hold on;
...
xlim([0 4.5]);set(gca,'XTick',[0:1:5]);
ylim([0 1.5]);set(gca,'YTick',[0:0.5:1.5]);
...
%% basic parameters of Reacted
fonts=12;initialMass = 2.3597e-4;
subplot(ndiagr,ndiagc,3,'position',locationsub(1,1,:));
for i = 1 : length(fileNumber)
    names=['E:\0 Comsol - t\Data\Donkoler_contrast\TCE-ReactedMass\',num2str(fileNumber(i)), '.txt'];
    tc = load(names);
    if i == 5
        names=['E:\0 Comsol - t\Data\Donkoler_contrast\TCE-ReactedMass\',num2str(fileNumber(i)), '-p.txt'];
        tc = load(names);
        plot(tc(:, 1)./tpv(i), (tc(:, 2) - tc(1,2))./initialMass,'linewidth',1.5,...
        'DisplayName',['No. ',num2str(i)]);
    else
        plot(tc(:, 2)./tpv(i), (tc(:, 3) - tc(1,3))./initialMass,'linewidth',1.5,...
        'DisplayName',['No. ',num2str(i)]);
    end
...
for i = 1 : length(fileNumber)
    names=['E:\0 Comsol - t\Data\Donkoler_contrast\NP-RetentionMassLowPM\',num2str(fileNumber(i)), '.txt'];
    tc = load(names);
    if i == 5
        names=['E:\0 Comsol - t\Data\Donkoler_contrast\NP-RetentionMassLowPM\',num2str(fileNumber(i)), '-p.txt'];
        tc = load(names);
        plot(tc(:, 1)./tpv(i), (tc(:, 2) - tc(1,2)),'linewidth',1.5,...
        'DisplayName',['No. ',num2str(i)]);
    else
        plot(tc(:, 2)./tpv(i), (tc(:, 3) - tc(1,3)),'linewidth',1.5,...
        'DisplayName',['No. ',num2str(i)]);
    end
